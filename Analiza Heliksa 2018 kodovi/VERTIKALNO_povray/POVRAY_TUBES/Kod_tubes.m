...
for i=L:N
    ...
    %if ((r(i).z/zz2)<2)
        if ((abs(r(i).z - 6.0446) < 1e-3) || (abs(r(i).z - 5.1811) < 1e-3))
            string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <0.000, 1.000, 0.000,',num2str(transp),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
        else
