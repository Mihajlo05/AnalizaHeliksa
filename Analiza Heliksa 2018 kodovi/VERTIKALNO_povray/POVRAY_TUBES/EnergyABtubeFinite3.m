function [r,m] = EnergyABtubeFinite3(Nring,N)

% MT AB tube FINITE for the dipoles' orientation: alfa = pi/3
    ...
    for j=1:Nr
       for i=1:(2*Nring)
         ...
         r(ind).z = 2*(j-Nr/2)*dz+dz*mod(i,2);
       end
    end
...
% Let's set the momentums of the dipoles
    for j=1:Nr
      for i=1:2*Nring
        ind = 2*(j-1)*Nring + i;
        m(ind).x = sin(alfa)*cos(pi*i/Nring + pi/2);
        m(ind).y = sin(alfa)*sin(pi*i/Nring + pi/2);
        m(ind).z = cos(alfa);
      end
    end
