...
for i=1:N
    ...
    transp = 1/(1+exp((0.6 - r(i).z/max(zz))/0.09));
    
    if (r(i).z/max(zz)<0.62)
...
%     
%     if (r(i).z/max(zz)>0.45 && r(i).z/max(zz)<0.5 )
%     string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <0.246, 0.203, 0.461,',num2str(0.8),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
%     elseif (r(i).z/max(zz)>0.43 && r(i).z/max(zz)<0.45 )
%     string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <0.246, 0.203, 0.461,',num2str(0.4),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
%     elseif (r(i).z/max(zz)<0.43)
%     string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <0.246, 0.203, 0.461,',num2str(transp),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
%     else
%     string3 ='';
%     end
