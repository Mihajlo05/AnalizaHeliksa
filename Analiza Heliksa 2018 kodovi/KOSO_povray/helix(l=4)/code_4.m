...
string4 = 'light_source { < 37,20,0> color rgb<0.5,0.5,0.5> shadowless}';
...
string7 = 'light_source { <-37,20,0> color rgb<0.5,0.5,0.5> shadowless}';
...
string10 = strcat('camera { orthographic location <',num2str(vec1(1)),',',num2str(vec1(2)),',',num2str(vec1(3)),'> look_at  <',num2str(vec2(1)),',',num2str(vec2(2)),',',num2str(vec2(3)),'> }');
...
for i=1:N
    ...
    transp = 1/(1+exp((0.5 - r(i).z/max(zz))/0.09));
    if (r(i).z/max(zz) < 0.48)
    tn(i) = transp;
    string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <',num2str(col(i).r),', ',num2str(col(i).g),', ',num2str(col(i).b),', ',num2str(transp),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
    else
    string3 = '';
    end
    
    string4 = strcat('cone { z',num2str(i),'+0.15*dip',num2str(i),' .12 z',num2str(i),'+0.5*dip',num2str(i),', .01 texture { pigment {color rgb<0.2,0.2,0.2>} finish {phong 0.2} } no_shadow }');
    string5 = strcat('cylinder { z',num2str(i),'+0.25*dip',num2str(i),' ,z',num2str(i),'-0.49*dip',num2str(i),' 0.04 texture { pigment {color rgb<0.2,0.2,0.2>} finish {phong 0.2} } no_shadow }');
    ...
    if (r(i).z/max(zz) < 0.52)
    string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <',num2str(col(i).r),', ',num2str(col(i).g),', ',num2str(col(i).b),', ',num2str(tn(i - k*(N/4))),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
    else
    string3 = '';
    end
...
stringD = strcat('cylinder { <0,0,',num2str(min(zz) - 0.5),'>,','<0,0,',num2str(max(zz) + 0.7),'>,',num2str(R - 0.5));
