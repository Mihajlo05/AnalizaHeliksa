...
for i=1:N
    ...
    if (r(i).z/max(zz) < 0.7)
    string3 = strcat('sphere { z',num2str(i),' 0.5 texture { pigment { color rgbt <',num2str(col(i).r),', ',num2str(col(i).g),', ',num2str(col(i).b),', ',num2str(tn(i - N/2)),'> } finish { specular 0.2 roughness 0.03} } no_shadow }');
    else
    string3 = '';
    end
